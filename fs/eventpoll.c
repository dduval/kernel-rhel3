...
#define EP_ITEM_FROM_EPQUEUE(p) (((struct ep_pqueue *)(p))->epi)
...
struct ep_pqueue {
	/* Must be the first field, to match the struct poll_table: */
...
static void ep_poll_callback(wait_queue_t *wait);
		     ...
		     struct file *tfile, int fd)
{
	...
	epq.epi = epi;
	init_poll_funcptr((poll_table *) &epq, ep_ptable_queue_proc);
	 ...
	 */
	revents = tfile->f_op->poll(tfile, (poll_table *) &epq);
