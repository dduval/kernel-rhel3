...
resume:
	while ( next != &this_parent->d_subdirs ) {
		...
		if ( exp_leaves && !autofs4_empty_dir(dentry) ) {
			this_parent = dentry;
			goto repeat;
		}
		...
		if (atomic_read(&dentry->d_count) > 1 && !is_tree_busy(mnt, dentry)) {
			DPRINTK(("autofs4_expire: returning %p %.*s\n",
				 dentry, (int)dentry->d_name.len, dentry->d_name.name));
			/* Start from here next time */
			list_del(&this_parent->d_subdirs);
			list_add(&this_parent->d_subdirs, &dentry->d_child);
			dget(dentry);
	...
	if ( exp_leaves && this_parent != root ) {
		next = this_parent->d_child.next;
		this_parent = this_parent->d_parent;
		goto resume;
	}
			...
			struct autofs_sb_info *sbi, int *arg)
{
	...
	if ((dentry = autofs4_expire(sb, mnt, sbi, how)) != NULL) {
