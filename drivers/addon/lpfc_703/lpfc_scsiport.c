...
/*
 * $Id: lpfc_scsiport.c 1.57.1.2 2004/05/09 10:58:06EDT jselx Exp  $
 */
...
lpfc_scsi_cmd_start(LPFC_SCSI_BUF_t * lpfc_cmd)
{
	...
	/* This function will handle all mapping, LUN mapping, LUN masking,
	   etc. */
	...
	if ((lun_device->pTarget->targetFlags & FC_NPR_ACTIVE) ||
		...
		if ((lun_device->failMask & LPFC_DEV_FATAL_ERROR) ||
			...
			lpfc_os_return_scsi_cmd(phba, lpfc_cmd);
			return 0;
		}
	}
	...
	if (!(piocbq->iocb_flag & LPFC_IO_POLL)) {
		...
		lpfc_sched_submit_command(phba, lpfc_cmd);
	} else {
		    ...
		    uint32_t bus, uint32_t target, uint32_t flag)
{
	...
	if (ret) {
		...
		if (flag & LPFC_EXTERNAL_RESET) {
			...
			if (ptarget) {
				if (!lpfc_sched_flush_target
				    (phba, ptarget, LPFC_CMD_STATUS_ABORTED,
				     0)) {
...
void
lpfc_qfull_retry(unsigned long ptr)
{
	lpfcHBA_t *phba;
	...
	lpfc_sched_queue_command(phba, (LPFC_SCSI_BUF_t *) (clkData->clData1));
...
lpfc_os_prep_io(lpfcHBA_t * phba, LPFC_SCSI_BUF_t * lpfc_cmd)
{
	...
	if (use_sg) {
		...
		seg_cnt = pci_map_sg(phba->pcidev, sgel_p, use_sg,
		...
		for (i = 0; i < seg_cnt; i++) {
			...
			if (num_bde == max_bde) {
				if ((bmp =
				     (DMABUF_t *) kmalloc(sizeof (DMABUF_t),
							  GFP_ATOMIC)) == 0) {
					break;
				}
	...
	fcp_cmnd->fcpDl = be32_to_cpu(cnt);
...
lpfc_queuecommand(struct scsi_cmnd *cmnd, void (*done) (struct scsi_cmnd *))
{
	...
	if ((targetp = lpfc_find_target(phba, lpfc_cmd->scsi_target))) {
		if ((lpfc_cmd->scsi_lun >= targetp->max_lun) ||
		    ((targetp->pcontext == 0)
		     && !(targetp->targetFlags & FC_NPR_ACTIVE))) {
			lpfc_free_scsi_buf(lpfc_cmd);
			LPFC_DRVR_UNLOCK(phba, iflag);
		...
		if ((cmnd->cmnd[0] == FCP_SCSI_INQUIRY) &&
		    (!lpfc_valid_lun(targetp, lpfc_cmd->scsi_lun))) {
			int retcod;
			uint8_t *buf;
	...
	if (ret) {
		...
		if (ret < 0) {
			...
			if (atomic_read(&phba->cmnds_in_flight) == 0) {
				...
				/* tell the midlayer we can't take commands
				   right now */
				LPFC_DRVR_UNLOCK(phba, iflag);
				return (1);
			}
...
lpfc_abort_handler(struct scsi_cmnd *cmnd)
{
	...
	lpfc_printf_log(phba->brd_no, &lpfc_msgBlk0749,
			...
			lpfc_cmd->scsi_target, lpfc_cmd->scsi_lun, rc,
...
lpfc_os_return_scsi_cmd(lpfcHBA_t * phba, LPFC_SCSI_BUF_t * lpfc_cmd)
{
	...
	if (presult->action_flag) {
		...
		lpfc_printf_log(phba->brd_no, &lpfc_msgBlk0729,
				...
				(uint32_t) lpfc_cmd->scsi_lun, lpfc_cmd->status,
...
lpfc_os_fcp_err_handle(LPFC_SCSI_BUF_t * lpfc_cmd, lpfc_xlat_err_t * presult)
{
	...
	case SCSI_STAT_QUE_FULL:
		...
		presult->host_status = DID_OK;	/* clear 07 */
		break;
...
lpfc_iodone(lpfcHBA_t * phba, LPFC_SCSI_BUF_t * lpfc_cmd)
{
	...
	if ((lnx_cmnd->result) || (lpfc_cmd->fcp_rsp->rspSnsLen)) {
		lp = (uint32_t *) lnx_cmnd->sense_buffer;
		/* Iodone <target>/<lun> error <result> SNS <lp> <lp3> */
		lpfc_printf_log(phba->brd_no, &lpfc_msgBlk0710,
				lpfc_mes0710,
				lpfc_msgBlk0710.msgPreambleStr,
				lpfc_cmd->scsi_target,
				(uint32_t) lpfc_cmd->scsi_lun, lnx_cmnd->result,
				*lp, *(lp + 3), lnx_cmnd->retries,
				lnx_cmnd->resid);
	...
	datadir = lpfc_cmd->datadir;
	if (lnx_cmnd->use_sg) {
		pci_unmap_sg(phba->pcidev, lnx_cmnd->request_buffer,
