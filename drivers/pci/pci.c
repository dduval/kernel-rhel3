...
/**
 * pci_device_restart	-	restart a failed PCI device
 * @dev: PCI device to restart
 * @name: name to refer to it by
 * @queue: recovery tqueue
 *
 *  When a PCI device gets into a mess we can often recover it by
 *  using D3 state to effectively power cycle the interface and 
 *  faking hot unplug/replug. This function must be called from
 *  process context. Within an IRQ the caller should invoke 
 *  pci_disable_device and then schedule the recovery to occur
 *  in process context.
 *
 *  For 2.5 schedule_work makes that nice and easy for 2.4 its a 
 *  bit messier.
 */
 
int pci_device_restart(struct pci_dev *dev, char *name)
{
	struct pci_driver *driver = dev->driver;
	
	/* Fence the card */
	pci_disable_device(dev);
	printk(KERN_WARNING "%s: PCI device being restarted.\n", name);
	
	/* Fake a hot unplug - the real one removes the card resources
	...
	/* Off and wait and on and pray */
	if(pci_set_power_state(dev, 3) != -EIO)
	{
		/* PM supported so do the PM cycling too */
		set_current_state(TASK_UNINTERRUPTIBLE);
		schedule_timeout(HZ/5);
		pci_set_power_state(dev, 0);
	}
	
	/* We use the proper announce function because there
	   we can. The less magic the better. Assuming the device
	   came back ok this will make the device driver reconfigure
	   the hardware without tons of error code in each driver */
	   
	pci_announce_device(driver, dev);	
	printk(KERN_WARNING "%s: PCI device has been restarted.\n", name);
	return 0;
}

EXPORT_SYMBOL_GPL(pci_device_restart);
