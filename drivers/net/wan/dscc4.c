...
static int dscc4_open(struct net_device *dev)
{
	...
	if (dpriv->flags & FakeReset) {
		...
		scc_writel(EventsMask | 0x00000004, dpriv, dev, IMR);
...
/*
 * Et*nc's PCISYNC only allows clock generation on first two ports.
 * DSCC4 can do more. It depends on the manufacturer of the card.
 */
 ...
 *   dubious (0x00000033 should be considered for CCR0 for testing).
 * - if line rate is specified, clocks are assumed to be generated. A quartz
 *   must be available (on pin XTAL1). Modes 6b/7b are used. The choice between
 *   these depends on the required frequency scaling.
 ...
 *
 * The clock signals wiring can be (is ?) manufacturer dependant.
 *
	...
	*state &= ~Ccr0ClockMask;
	if (*bps) { /* Clock generated - required for DCE */
		...
		*bps = xtal / divider;
		...
		/*
		 * External clock - DTE
		 * "state" already reflects Clock mode 0a (0xzzzzzz00 for CCR0).
