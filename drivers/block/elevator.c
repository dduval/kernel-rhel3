...
static int compatible(struct request *req, struct buffer_head *bh,
			request_queue_t *q, int rw,
			int count, int max_sectors)
{
	if (q->head_active)
		return 0;
	if (req->waiting)
			 ...
			 int max_sectors)
{
	...
	if (q->last_request) {
		struct request *__rq = q->last_request;

		if (compatible(__rq, bh, q, rw, count, max_sectors)) {
