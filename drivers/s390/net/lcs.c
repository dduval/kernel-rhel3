 ...
 *
 *    $Revision: 1.1.2.1 $    $Date: 2002/03/29 00:14:37 $
 * 
...
/* Function prototypes */
int lcs_rxpacket(lcs_drvr_globals * drvr_globals, u8 * start_buff,
 ...
 */
#if LCS_BUG
#define DEVSTAT_IS_SUSPENDED(devstat) \
	((devstat->flag & DEVSTAT_SUSPENDED) != 0)
#else
#define DEVSTAT_IS_SUSPENDED(devstat) \
	((devstat->ii.irb.scsw.actl & SCSW_ACTL_SUSPENDED) != 0)
#endif
...
lcs_initreadccws(lcs_read_globals * read)
{
	...
	for (cnt = 0; cnt < LCS_NUM_RX_BUFFS; cnt++, ccws++) {
		...
		if (set_normalized_cda
		    (ccws, virt_to_phys((void *) curr_skb->data)))
			return -ENOMEM
#else
		curr_iobuff = read->iobuff[cnt];
		if (set_normalized_cda
		    (ccws, virt_to_phys((void *) curr_iobuff)))
			return -ENOMEM;
#endif
	}
	ccws->cmd_code = ccw_transfer_in_channel1;
...
lcs_handle_irq(int irq, devstat_t * devstat, struct pt_regs *regs)
{
	...
	if (drvr_globals->read == chan.read) {
		...
		case chan_busy:
			chan.write->chan_busy_state = chan_idle;
			if (DEVSTAT_IS_SUSPENDED(devstat))
			{
				switch (drvr_globals->state) {
				case lcs_doing_io:
				case lcs_requesting_channels_for_lancmds:
