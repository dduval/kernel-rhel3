                    ...
                    dasd_device_t *device)
{
        ...
        if (device->queue.head != NULL &&
            ...
            device->queue.head->status != CQR_STATUS_ERROR) {
                DEV_MESSAGE (KERN_DEBUG, device,
                             ...
                             "(enough requests in queue) "
                             "head %p, status %x next %p",
                             device->queue.head,
                             device->queue.head->status,
                             device->queue.head->next);
                return NULL;
        }
	...
	if (size_needed + cplength*sizeof(ccw1_t) <= PAGE_SIZE) {
        ...
        list_for_each_entry (lowmem, &device->lowmem_pool, list) {
                DEV_MESSAGE (KERN_DEBUG, device,
                             "Take lowmem page %p for request",
                             lowmem);
	...
	if (data_offset == 0) {
                ...
                list_for_each_entry (lowmem, &device->lowmem_pool, list) {
                        DEV_MESSAGE (KERN_DEBUG, device,
                                     "Take lowmem page %p for extra data",
                                     lowmem);
                        list_del (&lowmem->list);
	...
	if (ccw_offset == 0) {
                ...
                list_for_each_entry (lowmem, &device->lowmem_pool, list) {
                        DEV_MESSAGE (KERN_DEBUG, device,
                                     "Take lowmem page %p for channel program",
                                     lowmem);
                        list_del (&lowmem->list);
...
dasd_free_request (ccw_req_t *cqr, dasd_device_t* device)
{
        ...
        if (cqr->flags & CQR_FLAGS_LM_IDAL) {
                DEV_MESSAGE (KERN_DEBUG, device,
                             "Freeing lowmem idal page %p from request %p.",
                             cqr->lowmem_idal, cqr);
        ...
        if (cqr->flags & CQR_FLAGS_LM_CQR) { 
                ...
                if (size_needed + cqr->cplength * sizeof(ccw1_t) > PAGE_SIZE) {
                        ...
                        DEV_MESSAGE (KERN_DEBUG, device,
                                     "Freeing lowmem channel program page %p",
                                     cqr->cpaddr);
                        lowmem = (dasd_lowmem_t *) cqr->cpaddr;
                        list_add (&lowmem->list, &device->lowmem_pool);
                }
                DEV_MESSAGE (KERN_DEBUG, device,
                             "Freeing lowmem request page %p",
                             cqr);
                lowmem = (dasd_lowmem_t *) cqr;
                list_add (&lowmem->list, &device->lowmem_pool);
	} else {
                    ...
                    ((cqr->lowmem_idal_ptr + nridaws * sizeof(unsigned long)) > 
                     ((void *) cqr + PAGE_SIZE))) {
                        ...
                        if (device->queue.head != NULL &&
                            ...
                            device->queue.head->status != CQR_STATUS_ERROR) {
                                DEV_MESSAGE (KERN_DEBUG, device, "%s",
                                             "Refusing emergency idals for "
                                             "request ((enough requests in "
                                             "queue)");
                                return -ENOMEM;
                        }
                        ...
                        list_for_each_entry (lowmem, &device->lowmem_pool, 
                                             list) {
                                DEV_MESSAGE (KERN_DEBUG, device, 
                                             "take lowmem page %p for IDALs of "
                                             "request %p.",
                                             lowmem, cqr);
                                list_del (&lowmem->list);
