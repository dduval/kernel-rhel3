...
/*
 * io_flush(unsigned int value)
 * 
 * Similar to AIX function, it makes the CPU think the value
 * has actually been _used_. Call it after an IO read to
 * make sure the read was actually done (especially useful
 * for reads used for flushing posted writes)
 */
static inline void __io_flush(unsigned int value)
{
...
#define io_flush(val)	__io_flush((unsigned int)(val))
